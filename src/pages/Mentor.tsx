...
export default function Mentor() {
          ...
          <div key={idx} className="relative group aspect-square rounded-xl overflow-hidden border border-stone-200 bg-stone-50">
            {isImageFile(f) ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center p-1 text-center">
                <FileText size={20} className="text-red-900 mb-1" />
                <span className="text-[9px] font-bold text-stone-600 line-clamp-2 leading-tight break-all">{f.name}</span>
              </div>
            )}
            <button type="button" onClick={() => onRemove(idx)} className="absolute top-1 right-1 bg-stone-900/70 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition" title="Bỏ tệp này">
              <X size={12} />
            </button>
          </div>
        ))}
        ...
        {files.length > 0 && (
          <div className="flex flex-col gap-2 mt-1">
            {files.map((file, i) => {
              ...
              return (
                <a key={i} href={id ? `https://drive.google.com/file/d/${id}/view` : file.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-3 bg-stone-50 border border-stone-200 rounded-xl hover:border-red-900/40 hover:bg-red-50/50 transition w-full sm:w-max max-w-full">
  ...
  const handleReplySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const fresh = await api.call<{ mentor: Question[] }>('BOOTSTRAP'); const updatedQ = fresh.mentor.find((q: Question) => q.id === viewingPost.id);
      if (updatedQ) setViewingPost(updatedQ);
    } catch (err: any) { showToast(err.message, 'error'); } finally { setIsProcessing(false); }
  };
  ...
  const CommentNode = ({ reply, allReplies, depth = 0 }: { reply: Reply, allReplies: Reply[], depth?: number }) => {
    ...
    return (
      <div className={`mt-4 ${depth > 0 ? 'ml-4 sm:ml-8 border-l-2 border-stone-200 pl-4' : ''}`}>
        <div className={`p-4 rounded-2xl ${reply.isVerified ? 'bg-emerald-50 border border-emerald-200' : 'bg-white border border-stone-200 shadow-sm'}`}>
          <div className="flex justify-between items-start mb-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-stone-100 text-stone-800 rounded-full flex items-center justify-center font-bold text-xs uppercase border border-stone-200">{reply.authorName.charAt(0)}</div>
              <div><p className={`text-sm font-bold ${reply.isVerified ? 'text-emerald-900' : 'text-stone-800'}`}>{reply.authorName}</p><p className="text-[10px] text-stone-500">{formatTime(reply.time)}</p></div>
            </div>
          ...
          <p className="text-sm text-stone-800 whitespace-pre-wrap leading-relaxed">{reply.content}</p>
          {reply.attachments && reply.attachments.length > 0 && <MediaGallery attachments={reply.attachments} />}

          <div className="mt-3 flex items-center gap-4">
            <button onClick={() => setReplyingTo({ id: reply.id, name: reply.authorName })} className="flex items-center gap-1 text-xs font-bold text-stone-500 hover:text-red-900 transition"><ReplyIcon size={14} /> Phản hồi</button>
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div><h1 className="text-3xl font-serif font-bold text-stone-900">Cố vấn Học thuật</h1><p className="text-stone-500 mt-1">Hỏi đáp, đính kèm bài tập & thảo luận chuyên sâu</p></div>

        <div className="flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-1/3">
            <MagicCard className="p-6 sticky top-24 border-t-4 border-t-red-900/40">
              <div className="flex items-center gap-2 mb-4 text-red-900"><MessageCircleQuestion size={24} /><h2 className="text-xl font-bold font-serif">Đặt câu hỏi mới</h2></div>
              <form onSubmit={handleAsk} className="space-y-4">
                ...
                <label className="flex items-center justify-center gap-2 p-3 border-2 border-dashed border-stone-300 rounded-xl cursor-pointer hover:bg-stone-50 transition">
                  <Paperclip className="text-stone-400" size={18} /><span className="text-sm text-stone-600 font-medium">Đính kèm ảnh / Đề bài</span>
                ...
                <label className="flex items-center gap-2 cursor-pointer mt-2">
                  <input type="checkbox" checked={isAnonymous} onChange={e => setIsAnonymous(e.target.checked)} className="rounded border-stone-300 text-red-900 focus:ring-red-900" />
                  <span className="text-sm text-stone-600 font-medium">Hỏi ẩn danh</span>
                </label>
                <button type="submit" disabled={isProcessing || !questionText.trim()} className="btn-primary w-full shadow-red-900/20 mt-2">{isProcessing ? 'Đang tải lên...' : <><Send size={16} /> Gửi câu hỏi</>}</button>
              </form>
            </MagicCard>
          </div>
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-4">
            {questions.map(q => (
              <MagicCard key={q.id} className="cursor-pointer hover:border-red-900/40 hover:shadow-md transition overflow-hidden">
                  ...
                  <div className="p-5 border-b border-stone-100 flex-1">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-stone-800">{q.askerDisplay}</span>
                        <span className="text-xs text-stone-400">• {formatTime(q.time)}</span>
                      </div>
                      {(isGvcn || session?.username === q.askerRealId) && (
                        <button onClick={(e) => handleDelete(e, q.id)} disabled={isProcessing} className="text-stone-400 hover:text-red-600 transition p-1 z-10"><Trash2 size={16} /></button>
                      )}
                    </div>
                    <p className="text-stone-800 font-medium whitespace-pre-wrap leading-relaxed line-clamp-3 mb-2">{q.question}</p>
                    
                    {q.attachments && q.attachments.length > 0 && <MediaGallery attachments={q.attachments} />}
                  </div>
                  
                  <div className="px-5 py-3 bg-stone-50 flex justify-between items-center text-sm font-bold text-stone-500">
                    <span>{q.replies?.length || 0} bình luận & thảo luận</span><span className="text-red-900">Bấm xem toàn màn hình ➔</span>
                  </div>
                </div>
              </MagicCard>
            ))}
            {questions.length === 0 && <div className="p-10 text-center border-2 border-dashed border-stone-200 rounded-2xl text-stone-400 font-medium">Bảng cố vấn đang trống.</div>}
          </div>
        </div>
      </div>

      {viewingPost && (
        <div className="fixed inset-0 z-[100] bg-stone-100 flex flex-col animate-in fade-in zoom-in duration-200">
          <div className="h-16 bg-white border-b border-stone-200 px-4 lg:px-8 flex items-center justify-between flex-shrink-0 shadow-sm">
            <div className="flex items-center gap-4">
              <button onClick={() => { setViewingPost(null); setReplyingTo(null); }} className="p-2 -ml-2 text-stone-500 hover:text-red-900 hover:bg-red-50 rounded-full transition"><ArrowLeft size={24} /></button>
              <div className="hidden sm:flex items-center gap-3 border-l border-stone-200 pl-4">
                ...
                <span className="text-sm font-bold text-stone-800">Bài đăng của {viewingPost.askerDisplay}</span>
              </div>
            </div>
            <button onClick={() => { setViewingPost(null); setReplyingTo(null); }} className="p-2 text-stone-500 hover:text-red-900 bg-stone-50 hover:bg-red-50 rounded-full transition"><X size={20} /></button>
          </div>

          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            <div className="w-full lg:w-3/5 xl:w-2/3 h-full overflow-y-auto p-4 md:p-8 bg-stone-100 custom-scrollbar">
              <div className="max-w-4xl mx-auto bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-stone-200">
                <div className="flex sm:hidden items-center gap-2 mb-4 pb-4 border-b border-stone-100">
                  <span className={`px-2 py-0.5 text-xs font-bold rounded border uppercase ${getSubjectColor(viewingPost.subject)}`}>{viewingPost.subject}</span>
                  <span className="text-sm font-bold text-stone-800">{viewingPost.askerDisplay}</span>
                </div>
                <p className="text-stone-400 text-sm mb-4 font-medium">Đăng lúc {formatTime(viewingPost.time)}</p>
                <p className="text-lg md:text-xl text-stone-900 whitespace-pre-wrap leading-relaxed font-medium">{viewingPost.question}</p>
            ...
            <div className="w-full lg:w-2/5 xl:w-1/3 bg-stone-50 border-t lg:border-t-0 lg:border-l border-stone-200 flex flex-col h-[60vh] lg:h-full flex-shrink-0 z-10 shadow-[-10px_0_30px_rgba(0,0,0,0.05)]">
              <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar">
                <h3 className="font-bold text-stone-800 mb-6 flex items-center gap-2"><MessageCircleQuestion className="text-red-900" size={20} />{viewingPost.replies?.length || 0} Bình luận & Thảo luận</h3>
                <div className="space-y-2">
                  ...
                  {(!viewingPost.replies || viewingPost.replies.length === 0) && <div className="text-center p-8 text-stone-400 italic">Chưa có bình luận nào. Hãy là người đầu tiên!</div>}
                </div>
              </div>

              <div className="p-4 bg-white border-t border-stone-200 shadow-[0_-5px_15px_rgba(0,0,0,0.02)]">
                {replyingTo && (
                  <div className="flex justify-between items-center mb-3 px-3 py-2 bg-stone-100 rounded-lg text-sm text-stone-600 font-medium">
                    <span>Đang trả lời <span className="font-bold text-red-900">@{replyingTo.name}</span></span><button onClick={() => setReplyingTo(null)} className="text-stone-400 hover:text-red-600"><X size={16} /></button>
                  </div>
                )}
                {replyFiles.length > 0 && (
                  <div className="mb-3">
                ...
                <form onSubmit={handleReplySubmit} className="flex items-end gap-2">
                  <label className="p-3 text-stone-400 hover:text-red-900 bg-stone-50 hover:bg-red-50 rounded-xl cursor-pointer transition flex-shrink-0 border border-stone-200">
                  ...
                  <div className="flex-1 bg-stone-50 border border-stone-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-red-900/20 focus-within:border-red-900 transition flex">
                    ...
                    <button type="submit" disabled={isProcessing || (!replyText.trim() && replyFiles.length === 0)} className="p-3 text-red-900 hover:bg-red-50 font-bold transition disabled:opacity-50"><Send size={20} /></button>
                  </div>
                </form>
              </div>
            </div>
